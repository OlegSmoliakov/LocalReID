...
from SFSORT_mod import SFSORT, Track
...
class ObjectTracking:
    ...
    def __call__(self):
        ...
        log.info(f"Input source:\nResolution: {frame_width}x{frame_height}, FPS: {frame_rate}")

        time.sleep(0.2)  # give some time to open the camera on mac

        self.desired_fps = 30
        frame_duration = 1.0 / self.desired_fps

        if self.output_video:
            fourcc = cv2.VideoWriter_fourcc(*"mp4v")
            out = cv2.VideoWriter(
                self.output_video + ".mp4", fourcc, self.desired_fps, (frame_width, frame_height)
            )
