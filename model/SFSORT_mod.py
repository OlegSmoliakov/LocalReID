    ...
    def __init__(self, args):
        ...
        if args.frame_width:
            ...
            if args.horizontal_margin is not None:
                ...
                self.r_margin = self.clamp(
                    args.frame_width - args.horizontal_margin, 0, args.frame_width
                )
        ...
        if args.frame_height:
            ...
            if args.vertical_margin is not None:
                ...
                self.b_margin = self.clamp(
                    args.frame_height - args.vertical_margin, 0, args.frame_height
                )
    ...
    def linear_assignment(cost_matrix, thresh):
        ...
        if use_lap:
            ...
            unmatched_a = np.array(
                [i for i in range(cost_matrix.shape[0]) if i not in matched_rows]
            )
            unmatched_b = np.array(
                [j for j in range(cost_matrix.shape[1]) if j not in matched_cols]
            )
